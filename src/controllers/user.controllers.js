...
// check status of a user based on their phone number.
// checkpoint for the merchant onboarding flow
...
// export const addPartnerRole = asyncHandler(async (req,res) => {
...
export const addPartnerRole = asyncHandler(async (req,res) => {
    ...
    // frontend will send the role to be added in the request body.
    const {roleId} = req.body;

    if(!roleId){
        throw new ApiError(400,"A 'roleId' is required in the request body");
    }
    ...
    if(existingRole) throw new ApiError(409,"user already has the requested role.");

    // create new role in the assignment join table
    await prisma.userHasRoles.create({
        data : {
            user_id: userId,
            role_id: roleId
    ...
    return res.status(200).json(new ApiResponse(200,{},"Partner role added to your account successfully"));
